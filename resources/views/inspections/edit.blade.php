<x-layouts.app :title="__('Edit Inspection')">
    <x-container class="py-6 lg:py-8">
        ...
        <div class="max-w-3xl">
            <x-heading>{{ __('Edit Inspection') }}</x-heading>
            <x-text class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {{ __('Update inspection information') }}
            </x-text>
                ...
                <form method="POST" action="{{ route('inspections.update', $inspection) }}" enctype="multipart/form-data" class="px-4 py-5 sm:p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <x-select id="equipment_id" name="equipment_id" class="mt-1 block w-full" required>
                                @foreach ($equipment as $item)
                                    <option value="{{ $item->id }}" {{ old('equipment_id', $inspection->equipment_id) == $item->id ? 'selected' : '' }}>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <x-input id="inspection_type" class="mt-1 block w-full" type="text" name="inspection_type" :value="old('inspection_type', $inspection->inspection_type)" required />
