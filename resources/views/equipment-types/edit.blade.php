<x-layouts.app :title="__('Edit Equipment Type')">
    <x-container class="py-6 lg:py-8">
        <div class="mb-6">
            <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
                ...
                <a href="{{ route('equipment-types.show', $equipmentType) }}" class="hover:text-gray-900 dark:hover:text-gray-200">{{ $equipmentType->name }}</a>
                <x-phosphor-caret-right width="16" height="16" />
                <span class="text-gray-900 dark:text-gray-100">{{ __('Edit') }}</span>
            </div>
            <x-heading>{{ __('Edit Equipment Type') }}</x-heading>
            <x-text class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {{ __('Update equipment type information and parameters') }}
            </x-text>
        </div>
        ...
        <form action="{{ route('equipment-types.update', $equipmentType) }}" method="POST">
            ...
            <div class="bg-white dark:bg-gray-900 shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10 sm:rounded-xl">
                <div class="px-4 py-5 sm:p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="mt-4 space-y-4">
                            <div>
                                ...
                                <x-input type="text" name="name" id="name" value="{{ old('name', $equipmentType->name) }}" required />
                                <x-error for="name" />
                            </div>

                            <div>
                                <x-label for="description">{{ __('Description') }}</x-label>
                                <x-textarea name="description" id="description" rows="3">{{ old('description', $equipmentType->description) }}</x-textarea>
                    ...
                    <div>
                        ...
                        <div id="parameters-container" class="space-y-4">
                            @foreach ($equipmentType->parameters as $index => $parameter)
                                <div class="parameter-item bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700" data-index="{{ $index }}">
                                    <div class="flex justify-between items-start mb-3">
                                        <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100">Parameter {{ $index + 1 }}</h4>
                                    ...
                                    <div class="grid grid-cols-1 gap-4">
                                        <div>
                                            ...
                                            <input type="text"
                                                   ...
                                                   value="{{ old('parameters.' . $index . '.label', $parameter->label) }}"
                        ...
                        <div id="no-parameters" class="text-center py-8 bg-gray-50 dark:bg-gray-800 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-700" style="display: {{ $equipmentType->parameters->count() > 0 ? 'none' : 'block' }};">
                ...
                <div class="flex items-center justify-end gap-x-4 border-t border-gray-900/10 dark:border-white/10 px-4 py-4 sm:px-6">
                    <a href="{{ route('equipment-types.show', $equipmentType) }}" class="text-sm font-semibold leading-6 text-gray-900 dark:text-gray-100">{{ __('Cancel') }}</a>
                    <button type="submit" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                        {{ __('Update Equipment Type') }}
                    </button>
                </div>
            </div>
        </form>
    ...
    @push('scripts')
    <script>
        let parameterIndex = {{ $equipmentType->parameters->count() }};
